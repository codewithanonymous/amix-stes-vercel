import { products } from "@/config/products";
import { BlurReveal } from "@/components/fx/BlurReveal";
import { SectionHeading } from "@/components/fx/SectionHeading";
import { ProductEmbed } from "@/components/ProductEmbed";

export function LiveProducts() {
  return (
    <section id="products" className="relative py-24 md:py-32 bg-surface" aria-label="Live products">
      <div aria-hidden className="absolute inset-0 bg-iris-soft opacity-60" />
      <div className="relative">
        <BlurReveal>
          <SectionHeading
            eyebrow="Live & Real"
            title={<>Try Our <em className="italic gradient-text-static">Live Products</em></>}
            subtitle="Real, working products. Click any card to launch the live demo in-page."
          />
        </BlurReveal>

        <div className="mx-auto mt-14 grid max-w-7xl gap-6 px-4 lg:grid-cols-3">
          {products.map((p, i) => (
            <BlurReveal key={p.id} delay={i * 0.08}>
              <ProductEmbed product={p} />
            </BlurReveal>
          ))}
        </div>
      </div>
    </section>
  );
}
